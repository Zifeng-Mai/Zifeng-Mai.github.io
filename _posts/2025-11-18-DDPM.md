...
- $\mathcal{L}_\text{prior}$ 用于保证前向过程能收敛到先验分布 $p_{prior}$ 上。只要我们选择合适的 $\{\beta_i\}$，使得 $\alpha_L$ 足够小，此时有 $p(\cdot\mid x_0)\approx p_{prior}(\cdot)$ 即可。在实践中我们一般有下面几种选择方式：
  1. 线性调度：$\beta_i=\beta_{start}+\frac{i}{L}(\beta_{end}-\beta_{start})$；
  2. 余弦调度：$\overline{\alpha}_i=\cos^2\left( \frac{\pi}{2}\cdot\frac{i}{L} \right)$。
- $\mathcal{L}_\text{recon}$ 用于保证第一步的重建精度，我们一般通过蒙特卡洛估计来近似和优化这一项。在实践中，我们一般把这一步融合进入 $t=2,\dots,L$ 的噪声估计loss中来优化。
- $\mathcal{L}_\text{diffusion}$ 用于拉近变分分布 $p_\phi(x_{i-1}\mid x_i)$ 和真实反向转移核 $p(x_{i-1}\mid x_i)$ 之间的距离。正如我们之前介绍的，共有3中不同的实现方式，其中最常用的是预测噪声。
