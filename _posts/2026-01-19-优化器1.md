...
header-img: img/optimizer-cover.jpg
preview:    本文深入探讨了SGD和Adam优化器的原理、优缺点以及实际应用场景，帮助读者更好地理解深度学习中的优化算法。
catalog: true
tags:
    - Optimizer
    - SGD
...
这个结论也可以解释为什么Adam中一般都令 $\beta_{1}\lt\beta_{2}$。为了更加准确的估计Hessian，$\hat{v}_t$ 的滑动平均应该尽可能长期，因此 $\beta_{2}$ 应该很接近1。而动量 $\hat{m}_t$ 是梯度的滑动平均，如果太过于长期的话，结果会将接近 $g_{\theta^{*}}=0$，这反而不好，因此动量的滑动平均应该更加局部一些。
