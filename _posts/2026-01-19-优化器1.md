...
这个结论也可以解释为什么Adam中一般都令 $\beta_{1}\lt\beta_{2}$。为了更加准确的估计Hessian，$\hat{v}_{t}$ 的滑动平均应该尽可能长期，因此 $\beta_{2}$ 应该很接近1。而动量 $\hat{m}_{t}$ 是梯度的滑动平均，如果太过于长期的话，结果会将接近 $g_{\theta^{*}}=0$，这反而不好，因此动量的滑动平均应该更加局部一些。
