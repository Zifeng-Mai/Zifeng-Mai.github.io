...
![EBM](../img/pics/score function.png)
...
令 $u=s_\phi^{(i)},v=p_{data}$，并假设当 $R\rightarrow \infty$ 时，$|s_\phi^{(i)}(x)p_{data}(x)|\rightarrow 0$。
...
其中，$C=\frac{1}{2}\mathbb{E}_{x\sim p_{data}}\left[ \left\|s(x)\right\|_2^2 \right]$ 是与 $\phi$ 无关的常数。
